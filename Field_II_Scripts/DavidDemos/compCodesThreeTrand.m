...
%N_elements      = 3;              % Number of transducers
...
% (Make a cell that holds all the arrays)
rfMatTran1 = [];
rfMatTran2 = [];
rfMatTran3 = [];
...
for i = 1:size(codes,1)           
    ...
    rfMatTran1(i,:) = rf_data(:,1);
    rfMatTran2(i,:) = rf_data(:,2);
    rfMatTran3(i,:) = rf_data(:,3);
    
    % Plot RF data
    subplot(4,6,1+6*(i-1))
    plot(rf_data(:,1))
    title(sprintf('RF Trand 1 Code %i',i))
    
    subplot(4,6,2+6*(i-1))
    plot(rf_data(:,2))
    title(sprintf('RF Trand 2 Code %i',i))
    
    subplot(4,6,3+6*(i-1))
    plot(rf_data(:,3))
    title(sprintf('RF Trand 3 Code %i',i))
    
    % Calculate cross correlation with code    
    subplot(4,6,4+6*(i-1))
    ccfMat1(i,:) = xcorr(rf_data(:,1),currCode);
    plot(ccfMat1(i,:));
    title(sprintf('CCF Trand 1 Code %i',i)) 
    
    subplot(4,6,5+6*(i-1))
    ccfMat2(i,:) = xcorr(rf_data(:,2),currCode);
    plot(ccfMat2(i,:));
    title(sprintf('CCF Trand 2 Code %i',i)) 
    
    subplot(4,6,6+6*(i-1))
    ccfMat3(i,:) = xcorr(rf_data(:,3),currCode);
    plot(ccfMat2(i,:));
    title(sprintf('CCF Trand 3 Code %i',i)) 
...
subplot(4,6,4+6*(i-1))
plot(sum(ccfMat1))
title('CCF Sum Trand 1')

subplot(4,6,5+6*(i-1))
plot(sum(ccfMat2))
title('CCF Sum Trand 2')

subplot(4,6,6+6*(i-1))
plot(sum(ccfMat3))
title('CCF Sum Trand 3')
%% Time adjustment should go here
% Epic time adjustment code...

%% Sum time adjusted opinions of each transducer
subplot(4,6,1+6*(i-1))
plot(sum(ccfMat1) + sum(ccfMat2) +  sum(ccfMat3))
title('Sum of Opinions')
...
% hold on 
% plot(sum(ccfMat3))
title('Looking for Opinion Mismatch')
legend('Tran 1','Tran 2')%,'Tran 3')
% Tran 1 and 3 are the same, and tran 2 is different
% This means transducer 2 is in the middle
max12 = max(sum(ccfMat1) - sum(ccfMat2));
max13 = max(sum(ccfMat1) - sum(ccfMat3));
max23 = max(sum(ccfMat2) - sum(ccfMat3))
