...
function [apo_vector_tx,apo_vector_rx] = getApodization(xFocus,no_active_tx,no_active_rx,width, kerf, no_elements)    

    % How much we activate the active transducers
    % (could add Hamming/Hanning apodization to this, if desired)
     apo_tx = ones(1,no_active_tx);
     apo_rx = ones(1,no_active_rx);

    % Number of elements before those used for transmit
    N_pre_tx = round(xFocus/(width+kerf) + no_elements/2 - no_active_tx/2);
    if(N_pre_tx < 0)
        N_pre_tx = 0;
    end;
    ...
    if(N_post_tx < 0)
        N_post_tx = 0;
        N_pre_tx =  no_elements - (N_post_tx + no_active_tx);
    end    
    ...
    N_pre_rx = round(xFocus/(width+kerf) + no_elements/2 - no_active_rx/2);
    if(N_pre_rx < 0)
        N_pre_rx = 0;
    end;
    ...
    if(N_post_rx < 0)
        N_post_rx = 0;
        N_pre_rx =  no_elements - (N_post_rx + no_active_rx);
    end 
