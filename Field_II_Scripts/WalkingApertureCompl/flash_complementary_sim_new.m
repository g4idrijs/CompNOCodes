...
%     end
    ...
    code = impulse_response;
    xdc_focus_times(xmt, 0, zeros(1, no_elements));
    xdc_focus(xmt, 0, [0 0 60/1000]);
    
    arrayLength = pitch*no_elements-kerf;
    xElements = -(arrayLength/2+width/2):pitch:(arrayLength/2-width/2);
    delays = -(sqrt((xElements-0).^2+(60/1000)^2))/c;
    
    xdc_focus_times(xmt, 0, delays);
...
bf_temp = bft_beamform(Tmin, rf_data(1:no_rf_samples, :));
