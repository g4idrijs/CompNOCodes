...
%% Set up multiple transmit pairs
% Set receive focus in each of these cases
no_lines = 3; % Number of lines per transmit pair
zDepth = 40/4000; % (m) Where we focus in z

lineWidth = 0.1/1000; % (m) Width of each line
jumpBetweenSets = 6/1000; % (m) How far to jump between
numTransPairs = jumpBetweenSets/lineWidth;

startX = -8/1000; % Starting point for receive focus
...
% Stores composite image
% Each transmit pair result is in a differenet 3rd dimension
compImag = zeros(no_rf_samples, no_lines,numTransPairs); % Composite image

for currLineSet = 1:numTransPairs        
    ...
    % Set receive and transmit focus points
    % Each row is a point in the form (x,y,z)     
    reFocus = zeros(no_lines,3);
    for i = 1:no_lines % no_lines per transmit pair
       reFocus(i,:) = [startX+jumpBetweenSets*(i-1)+(currLineSet-1)*lineWidth, 0,zDepth];
    end    
    
    % Set transmit focus locations (same as receive focus)
    transFocLocs = reFocus;  
    
    % Set code transmit locations
    numTransFocLocs = no_lines;            
...
%     caxis([-55 0]);
%     
    compImag(:,:,currLineSet) = bfImag;

end
field_end
bft_end

% Combine the lines created on the different transmits
% Each set of line data is in a different 3rd dimension index of compToPlot
compToPlot = zeros(no_rf_samples,no_lines*numTransPairs);
currLine = 1;
for j = 1:no_lines
    for i = 1:numTransPairs
        compToPlot(:,currLine) = compImag(:,j,i);
        currLine = currLine + 1;
    end
end

% Normalize and do envelope detection
env_compToPlot = abs(hilbert(compToPlot));
env_compToPlot = env_compToPlot / max(max(env_compToPlot));
toPlotFinal = 20*log10(env_compToPlot+eps);

imagesc([startX max(reFocus(:,1))]*1000, [Rmin Rmax]*1000, toPlotFinal);
