...
pointSpotsX = -8e-3:2e-4:-4e-3; % Phantom/aligned aperture positions

% Hold vector of start times from calc_scat_multi
% (holds one time for each phantom / aligned aperture pair)
start_timeCodeVect = [];

% Store what row our first nonzero data arrived at
firstNonZRowIndVect = [];

% Keep track of the element used to fire
usedElVect = [];
...
for pointCount = 1:numel(pointSpotsX)
    ...
    % Set the transmit focus location (each row is an (x,y,z) point)
    % for this loop
    zTransDepth = 40e-3; % (m)

    xTransFocusVals = pointSpotsX(pointCount);
    ...
    % Current phantom position 
    pht_pos = [-7e-3 0 20e-3
                -5e-3 0 20e-3
        ];                
    ...
    fs = 1000e6;            %  Sampling frequency                       [Hz]
    ...
    for transFocInd = 1:no_lines
        ...
        % Keep track of the element used to fire
        usedEl = find(apo_vector_rx);
        usedElVect = [usedElVect usedEl];
    ...
    %     pause(0.05)
        ...
        for currTransmit = 1:2                   
            ...
            if(currTransmit == 1)            
                ...
                firstNonZRowInd = find(sum(abs(codeRF),2)~=0,1);
                firstNonZRowIndVect = [firstNonZRowIndVect firstNonZRowInd];
    %             figure            
    %             imagesc(codeRF(600:end,:))
    %             title(sprintf('Received RF data at %1.2d',xFocus))
            else
                %...second code in pair
...
figure
plot(usedElVect,firstNonZRowIndVect,'o')
title(sprintf('Nonzero data start row: Phantom at 20e-3 m'))
ylabel('Row in collection')
xlabel('Firing element')
figure
plot(usedElVect,start_timeCodeVect,'o')
title(sprintf('Start Time: Phantom at 20e-3 m'))
ylabel('Start time (s)')
xlabel('Firing element')
% Release memory
field_end
