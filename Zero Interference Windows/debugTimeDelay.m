% Vary phantom position, and see how start time changes 
...
figure
% Go through each phantom position
phtDepths = [20e-3:1e-3:40e-3];
for phtCount = 1:numel(phtDepths)
    clearvars -global -except phtDepths phtCount
    centApoVect = []; % Central element of apodization
    
    % Hold vector of start times from calc_scat_multi
    % (holds one time for each phantom / aligned aperture pair)
    start_timeCodeVect = [];
    
% Go through each aperture position
for pointCount = 1:numel(pointSpotsX)
    ...
    pht_pos = [phtX 0 phtDepths(phtCount)];
    ...
    for transFocInd = 1:no_lines
        ...
        [~,apo_vectorCent] = getApodization(xFocus,1,1,width, kerf, no_elements);
        centApo = find(apo_vectorCent);
        centApoVect = [centApoVect centApo];
    ...
    %     pause(0.05)
        ...
        % Transmit focus 
        xdc_focus(xmt, 0, transFoc);  % Do transmit focus
        %xdc_focus_times(xmt, 0, zeros(1,no_elements)); % Don't transmit focus
        ...
        for currTransmit = 1:2                   
            ...
            if(currTransmit == 1)            
                ...
                % Grab focus delays when we focus in the middle
                if(xFocus == 0)
                    xmtDelay = xdc_get(xmt, 'focus');
                end
...
% Plot start_time as a function of position along aperture
plot(centApoVect,start_timeCodeVect )
hold on
end

xlabel('Transducer element # Aligned with Focus Center')
ylabel('Calc\_scat\_multi start time (s)')
title('start\_time with Sliding Subaperture as Scatterer Depth Varied')
legend(strsplit(num2str(phtDepths)))
% % Plot focus delays when we focus in the middle
% figure
% relFocDelay = xmtDelay(centApoVect);
% plot(centApoVect,relFocDelay)
% title('Transmit Delays: Focus at (0,0,40e-3) m. Phantom depth 40 mm') 
% xlabel('Transducer element #')
% ylabel('Focus Delay (s)')

% Try and paste focus delays onto start_time
% adjFocToStart = -relFocDelay;
% adjFocToStart = adjFocToStart + min(adjFocToStart);
% figure
% plot([adjFocToStart,adjFocToStart])
% title(sprintf('start_time (as phantom, focus, subaperture moves) related to \nfocus delays (entire aperture focused in center)'),'FontSize',10,'Interpreter','none')
% xlabel('Transducer element # (center of aperture for start\_time)')
% ylabel('Focus Delay (s)')
% legend('Adjusted focus delays','start\_time')
% Release memory
field_end
bft_end
